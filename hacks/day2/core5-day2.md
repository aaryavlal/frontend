...
|---------|-------|-------------|
| **INPUT** | 2730, 2681-2715 | Task creation, drag & drop, button clicks |
| **OUTPUT** | 2764-2782, 2805 | Results text, visual panel, alerts |
| **PROCEDURE** | 2747-2785 | `computeSpeedup()` - 9 sequential steps |
| **SEQUENCING** | 2747-2785 | Collect → validate → calculate → display → store |
| **SELECTION** | 2755, 2763, 2770 | `if` checks, ternary operators |
| **ITERATION** | 2749-2753, 2760 | `.filter()`, `.map()`, `.reduce()` on arrays |
| **LIST** | 2749, 2666 | `seriesBlocks[]`, `parallelBlocks[]`, `savedRuns[]` |

**Algorithm:** Serial = sum all tasks. Parallel = series sum + max(parallel). Speedup = serial/parallel.
